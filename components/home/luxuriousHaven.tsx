...
const getCards = (t: any) => [
  {
    icon: <FaBuilding size={32} className="md:w-9 md:h-9" />,
  ...
  {
    icon: <FaUsers size={32} className="md:w-9 md:h-9" />,
  ...
  {
    icon: <FaStar size={32} className="md:w-9 md:h-9" />,
  ...
  {
    icon: <FaLightbulb size={32} className="md:w-9 md:h-9" />,
  ...
  {
    icon: <FaCheckCircle size={32} className="md:w-9 md:h-9" />,
  ...
  {
    icon: <FaHeadset size={32} className="md:w-9 md:h-9" />,
...
export default function WhatWeOffer() {
  ...
  return (
    <section ref={sectionRef} className="relative w-full min-h-screen select-none bg-white py-12 md:py-14 overflow-hidden" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <motion.div
        style={{ x: textX }}
        className="absolute top-10 left-0 whitespace-nowrap select-none pointer-events-none opacity-[0.03] z-0"
      >
        <h2 className="text-[6rem] sm:text-[10rem] md:text-[14rem] lg:text-[20rem] font-black text-slate-900 tracking-tighter uppercase">
          {t.services?.banner || 'HOROHOUSE • SERVICES • AFRICA • HOROHOUSE • SERVICES • AFRICA'}
        </h2>
      </motion.div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 lg:items-end mb-12 md:mb-16">
          <div className="flex-1">
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-8 md:w-12 h-1 bg-blue-500" />
                <span className="text-blue-500 font-black text-[10px] md:text-xs uppercase tracking-[0.3em] md:tracking-[0.4em]">{t.services?.title || 'Our Services'}</span>
              </div>
              <h2 className="text-4xl sm:text-5xl md:text-7xl font-black text-slate-900 leading-[0.9] tracking-tighter uppercase">
          ...
          <div className="flex-1 lg:pb-3">
            <motion.p
              ...
              className="text-slate-500 text-lg sm:text-xl md:text-2xl leading-tight max-w-sm border-l-4 border-blue-500 pl-4 md:pl-6"
            >
        ...
        <div className="relative">
          <Carousel
            ...
            className="w-full"
          >
            <CarouselContent className="-ml-4 md:-ml-6 py-4 md:py-8">
              {cards.map((card, i) => {
                ...
                return (
                  <CarouselItem key={i} className="pl-4 md:pl-6 basis-auto">
                    <motion.div
                      ...
                      whileHover={{ y: -8 }}
                      // CHANGED: Fixed responsive sizing from w-12 md:w-80 to w-[75vw] sm:w-[300px] md:w-80
                      className={`w-[75vw] max-w-[320px] sm:w-[300px] md:w-80 h-[280px] md:h-[300px] cursor-pointer transition-all duration-500 rounded-[2rem] md:rounded-[2.5rem] relative overflow-hidden group ${
                        isActive ? "shadow-2xl scale-[1.02] md:scale-105" : "shadow-lg hover:shadow-xl"
                      }`}
                      onClick={() => scrollTo(i)}
                      role="button"
                    ...
                    >
                      ...
                      <div
                        className={`absolute inset-0 bg-gradient-to-br ${card.gradient} transition-opacity duration-500 ${
                          isActive ? "opacity-30 md:opacity-20" : "opacity-10 group-hover:opacity-15"
                        }`}
                        aria-hidden="true"
                      />
                      ...
                      <div className="relative z-10 h-full flex flex-col justify-between p-6 md:p-8">
                        <div>
                          ...
                          <motion.div
                            transition={{ duration: 0.6 }}
                            className="inline-flex items-center justify-center w-12 h-12 md:w-16 md:h-16 mb-4 md:mb-5 rounded-xl md:rounded-2xl bg-white/15 text-white shadow-lg border border-white/10"
                            aria-hidden="true"
                          >
                            {card.icon}
                          </motion.div>
                          ...
                          {/* Description */}
                          <p className="text-xs md:text-sm leading-relaxed text-white/90 line-clamp-3">
                            {card.desc}
                          </p>
                        </div>
                        ...
                        {isActive && (
                          <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="absolute top-5 right-5 md:top-6 md:right-6 w-2.5 h-2.5 md:w-3 md:h-3 bg-white rounded-full shadow-lg"
                        ...
                        <div className="absolute bottom-0 right-0 w-16 h-16 md:w-24 md:h-24 bg-white/5 rounded-tl-full" aria-hidden="true" />
                      </div>
                    </motion.div>
                  </CarouselItem>
                );
          ...
          {/* Navigation dots */}
          <div className="flex justify-center gap-2 mt-8 md:mt-6" role="tablist" aria-label="Service navigation">
            {cards.map((card, i) => (
              <button
                key={i}
                onClick={() => scrollTo(i)}
          ...
          <motion.div
            ...
            className="text-center mt-4 md:mt-6 text-slate-500 text-xs md:text-sm lg:hidden"
            aria-hidden="true"
          >
