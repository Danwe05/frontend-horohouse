...
import { Search, MapPin, Star, ShieldCheck, HeadphonesIcon, Tag } from "lucide-react";
...
export default function StudentIndexClient({ initialData, initialStats, translations }: any) {
  const s = translations.students?.page || {};
  ...
  useEffect(() => {
    if (page !== 1 || Object.keys(filters).length > 2) {
      ...
      <main className="max-w-[1440px] mx-auto px-6 sm:px-10 py-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-6 border-b border-[#EBEBEB]">
          <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <p className="text-[#717171] text-[16px]">{(s.showingVerifications || "Showing {count} verified listings").replace("{count}", total.toString())}</p>
          </motion.div>
