...
export default function CommunityPostClient({
  ...
  const [replies, setReplies] = useState<Reply[]>(buildOneLevel(initialReplies, initialPost.id));
  const [related] = useState<Post[]>(initialRelated);
  ...
  useEffect(() => {
    ...
    apiClient.incrementCommunityPostView(initialPost.id);
  }, [initialPost.id]);
  ...
  const handlePinToggle = async () => {
    try {
      ...
      setPost(prev => ({ ...prev, pinned: !prev.pinned }));
    ...
    <div className="min-h-screen bg-white text-[#222222] font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-10">
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12 items-start">
          <main className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-6 py-5 border-t border-b border-[#EBEBEB] mb-10">
              ...
              <div className="flex items-center gap-2 text-[15px] font-semibold text-[#222222]">
                ...
                {post.replyCount.toLocaleString()} Replies
              </div>
              <button
                className="flex items-center gap-2 text-[15px] font-semibold text-[#222222] hover:text-blue-600 transition-colors ml-auto md:ml-0"
                onClick={handleShare}
              >
              ...
              <button
                className="hidden md:flex items-center gap-2 text-[15px] font-semibold text-[#222222] hover:text-blue-600 transition-colors"
                onClick={handleFlag}
                disabled={flagging}
              >
            ...
            <section className="mb-12">
              <h2 className="text-[20px] md:text-[22px] font-bold text-[#222222] mb-6 md:mb-8">
                {post.replyCount} {post.replyCount === 1 ? "Reply" : "Replies"}
              </h2>

              {user ? (
                <div className="mb-8 md:mb-10 flex gap-3 md:gap-4 items-start">
                  <Avatar className="h-9 w-9 md:h-10 md:w-10 shrink-0">
                    <AvatarFallback className="bg-[#222222] text-white font-bold">
                      {user.name?.[0]?.toUpperCase() ?? "U"}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1">
                    <textarea
                      ...
                      rows={3}
                      className="w-full resize-none text-[15px] md:text-[16px] text-[#222222] placeholder:text-[#B0B0B0] focus:outline-none leading-relaxed px-4 py-3 md:py-4 border border-[#DDDDDD] rounded-xl md:rounded-2xl focus:border-[#222222] focus:ring-1 focus:ring-[#222222] bg-[#F7F7F7]"
                    />
                    <div className="flex justify-end mt-3">
                      <button
                        onClick={submitReply}
                        disabled={submitting || !replyText.trim()}
                        className="px-6 md:px-8 py-2 md:py-3 bg-[#222222] hover:bg-black text-white text-[14px] md:text-[15px] font-semibold rounded-xl transition-colors flex items-center gap-2 disabled:opacity-50"
                      >
                ...
                <div className="mb-10 p-6 md:p-8 rounded-2xl bg-[#F7F7F7] border border-dashed border-[#DDDDDD] text-center">
                  <p className="text-[15px] md:text-[16px] text-[#717171] mb-4">You must be signed in to join the discussion.</p>
                  <Link
                    href="/auth/login"
                    className="inline-block px-6 md:px-8 py-2 md:py-3 bg-[#222222] hover:bg-black text-white text-[14px] md:text-[15px] font-semibold rounded-xl transition-colors"
                  >
                    Sign In
                  </Link>
                </div>
          ...
          <aside className="w-full lg:w-[320px] shrink-0 space-y-8 lg:sticky lg:top-28 pb-10">
            <div className="p-6 md:p-8 rounded-2xl border border-[#EBEBEB] bg-[#F7F7F7]">
              <h3 className="text-[16px] md:text-[18px] font-bold text-[#222222] mb-4">Guidelines</h3>
              <ul className="space-y-3">
                {[
                  "Be respectful and supportive",
                  "No spam or self-promotion",
                  "Keep it relevant to hosting",
                  "Protect your privacy"
                ].map(item => (
                  <li key={item} className="flex gap-2.5 text-[13px] md:text-[14px] text-[#484848] leading-tight">
                    <CheckCircle2 className="w-4 h-4 text-[#717171] shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
            ...
            {related.length > 0 && (
              <div>
                <h3 className="text-[16px] md:text-[18px] font-bold text-[#222222] mb-5">Related discussions</h3>
                <div className="space-y-4">
                  {related.map(r => (
