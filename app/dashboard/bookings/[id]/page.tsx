...
import {
    ...
    XCircle, RotateCcw, Loader2, BedDouble, Wifi, UtensilsCrossed,
    ParkingCircle, Tv, DollarSign, Sparkles, AlertCircle, Phone, Mail,
...
const BookingTimeline = ({ status, checkIn }: { status: BookingStatus; checkIn: string }) => {
                    ...
                    <main className="p-4 md:p-8 lg:p-12 max-w-7xl mx-auto w-full">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                            <div className="flex items-center gap-4">
                                <Button variant="ghost" size="icon" className="rounded-2xl bg-white border border-slate-200 shadow-sm" onClick={() => router.push('/dashboard/bookings')}>
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                ...
                                <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
                                    <div className="relative h-[400px]">
                                        <img src={prop?.images?.[0]?.url} className="w-full h-full object-cover" alt="" />
                                        ...
                                        <div className="absolute bottom-8 left-8 right-8 text-white">
                                            ...
                                            <p className="flex items-center gap-2 text-slate-200 text-sm font-medium">
                                                <MapPin className="h-4 w-4 text-indigo-400" /> {prop?.address}, {prop?.city}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-8 grid grid-cols-2 md:grid-cols-4 gap-6 border-b border-slate-100 bg-slate-50/50">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-xs text-indigo-600 font-bold">Resort View</p>
                                        </div>
                                    </div>

                                    {/* Timeline Section */}
                                    <div className="p-8">
                                        <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-8 text-center">Reservation Lifecycle</h3>
                                        <BookingTimeline status={booking.status} checkIn={booking.checkIn} />
                                    </div>
                                </div>
                                ...
                                <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
                                    <div className="flex items-center justify-between mb-8">
                                        ...
                                        <Badge variant="outline" className="rounded-xl px-3 py-1 text-xs font-bold text-slate-400">Verified Amenities</Badge>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-4">
                                        {prop?.shortTermAmenities?.hasWifi && <div className="flex items-center gap-3 text-sm font-bold text-slate-600"><Wifi className="h-5 w-5 text-indigo-500" /> High-speed WiFi</div>}
                                        {prop?.shortTermAmenities?.hasAirConditioning && <div className="flex items-center gap-3 text-sm font-bold text-slate-600"><Wind className="h-5 w-5 text-indigo-500" /> Air Conditioning</div>}
                                        {prop?.shortTermAmenities?.hasParking && <div className="flex items-center gap-3 text-sm font-bold text-slate-600"><Car className="h-5 w-5 text-indigo-500" /> Free Parking</div>}
                                        {prop?.shortTermAmenities?.hasKitchen && <div className="flex items-center gap-3 text-sm font-bold text-slate-600"><Utensils className="h-5 w-5 text-indigo-500" /> Full Kitchen</div>}
                                        {prop?.shortTermAmenities?.hasWasher && <div className="flex items-center gap-3 text-sm font-bold text-slate-600"><WashingMachine className="h-5 w-5 text-indigo-500" /> In-unit Washer</div>}
                                        <div className="flex items-center gap-3 text-sm font-bold text-slate-600 opacity-50"><ShieldCheck className="h-5 w-5 text-indigo-500" /> Smoke Alarm</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6 lg:sticky lg:top-8 self-start">
                                ...
                                <div className="bg-white rounded-[2.5rem] border-2 border-slate-900 p-8 shadow-2xl shadow-indigo-100">
                                    ...
                                    <div className="space-y-4 mb-8">
                                        <div className="flex justify-between text-sm font-bold">
                                            <span className="text-slate-500">{booking.priceBreakdown?.pricePerNight.toLocaleString()} x {nights} nights</span>
                                            <span className="text-slate-900">{booking.priceBreakdown?.subtotal.toLocaleString()} {booking.currency}</span>
                                        </div>
                                        <div className="flex justify-between text-sm font-bold">
                                            <span className="text-slate-500">Service Fee</span>
                                            <span className="text-slate-900">{booking.priceBreakdown?.serviceFee.toLocaleString()} {booking.currency}</span>
                                        </div>
                                    ...
                                    <div className={`rounded-2xl p-4 mb-8 flex items-center justify-between ${booking.paymentStatus === 'paid' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                                        ...
                                        <span className="text-[10px] font-bold opacity-60">REF: {booking.paymentReference || 'N/A'}</span>
                                    </div>
                                ...
                                <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <Button variant="secondary" className="rounded-xl h-12 bg-slate-50 border border-slate-100 font-bold text-slate-700" asChild>
                                            <a href={`tel:${booking.hostId?.phoneNumber}`}><Phone className="h-4 w-4 mr-2 text-indigo-500" /> Call</a>
                                        </Button>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white rounded-[2.5rem] border border-slate-200 p-4 shadow-sm group cursor-pointer overflow-hidden">
                                    <div className="h-40 bg-slate-100 rounded-[1.5rem] flex items-center justify-center flex-col gap-2 transition-colors group-hover:bg-slate-200">
                                        ...
                                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Open in Google Maps</p>
                                    </div>
                                </div>
