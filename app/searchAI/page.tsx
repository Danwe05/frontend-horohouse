...
export default function AiChatPage() {
  ...
  const speakMessage = useCallback((text: string) => {
    if (!('speechSynthesis' in window) || !text || !voiceEnabled || !speechInitialized) {
      return
    }
    ...
    if (messages.length > 1) {
      setConversations(prev => {
        ...
        if (!currentConv) {
          ...
          return prev.map(c => 
            c.id === sessionId 
              ? { ...c, messageCount: messages.length, timestamp: new Date() }
              : c
          )
        }
      })
  ...
  useEffect(() => {
    ...
    if (messages.length > previousMessagesLengthRef.current) {
      ...
      if (newAssistantMessages.length > 0) {
        ...
        if (latestAssistantMessage.content && latestAssistantMessage.content.trim()) {
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    if (!isAuthenticated) {
      toast.error("Non authentifié", {
        description: "Vous devez vous connecter pour envoyer des messages",
      })
      return
    }
  ...
  const handleSelectConversation = useCallback((id: string) => {
    ...
    toast.success("Conversation chargée", {
      description: "La conversation a été chargée avec succès",
    })
  }, [loadSession])
  ...
  const handleToggleVoice = useCallback(() => {
    if (!voiceEnabled && !speechInitialized) {
      initializeSpeechSynthesis()
    }
    ...
    toast.success(newValue ? "Voix activée" : "Voix désactivée", {
      description: newValue 
        ? "Les réponses seront lues à haute voix" 
        : "Les réponses ne seront plus lues à haute voix",
    })
