...
import {
  ...
  BadgeCheck, ChevronRight, Globe, Clock,
  ArrowLeft,
} from "lucide-react";
...
export default function CommunityAuthorClient({ 
  initialProfile, 
  initialPosts 
}: { 
  initialProfile: AuthorProfileData; 
  initialPosts: Post[];
}) {
  const [activeTab, setActiveTab] = useState("posts");
  const { user, communityStats } = initialProfile;
        ...
        {activeTab === "posts" && (
          <div>
            {initialPosts.length === 0 ? (
              ...
              <div>
                {initialPosts.map((p: Post) => <MiniPostRow key={p.id} post={p} />)}
              </div>
            )}
          </div>
        )}
